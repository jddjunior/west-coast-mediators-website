...
| Static HTML export (for PHP server hosting) | `/html-export-next` | Complete — ready to upload to server |
...
- Replaced both mediator portraits throughout the Next.js app and `html-export-next/` with the provided Builder CDN images:
...
### Static HTML Export (`/html-export-next`)
...
Copy `html-export-next/` contents to the web root:

```
html-export-next/index.html              → /public_html/index.html
html-export-next/schedule/               → /public_html/schedule/
html-export-next/assets/                 → /public_html/assets/
```
...
5. Update the iframe `src` in `html-export-next/schedule/kevin/index.html` from the placeholder URL to the live URL
